...
public class SequenceRenderer {
    ...
    private static readonly Color SELECTION_BOX_COLOR = new(0f, 0.5f, 1f, 0.25f);
    ...
    public SequenceRenderer(int windowWidth, int windowHeight, int columnCount) {
        ...
        columnWidth = canvasRect.width / columnCount;
        secondsPerPixel = TIMESPAN / canvasRect.height;
        pixelsPerSecond = canvasRect.height / TIMESPAN;
        ...
        controlCurveTexture = new Texture2D(columnWidth - 2 * CONTROL_CURVE_PADDING, canvasRect.height, TextureFormat.RGBA32, false);
    ...
    private void DrawWindow(SequenceRenderInput input) {
        ...
        DrawRect(canvasRect.x, canvasRect.y, canvasRect.width, canvasRect.height, BACKING_COLOR, true);
        ...
        switch (editorState.Mode) {
            ...
            case SequenceEditorMode.OnOffEvents when Event.current.type == EventType.Repaint:
                DrawOnOffEvents(time, timeAsFloat, beatArray, sequence.OnOffEvents, background.EventLabels, editorState);
                break;
            case SequenceEditorMode.ControlCurves when Event.current.type == EventType.Repaint:
                DrawControlCurves(time, timeAsFloat, beatArray, sequence.ControlCurves, background.CurveLabels, editorState);
                break;
        }
    ...
    private void DrawOnOffEvents(long time, float timeAsFloat, float[] beatArray, IReadOnlySequence<OnOffEvent> onOffEvents,
        IReadOnlyList<string> labels, SequenceEditorState state) {
        DrawGrid(timeAsFloat, beatArray, state.Column, state.FirstColumnIndex);

        for (int i = 0, j = state.FirstColumnIndex; i < columnCount && j < labels.Count; i++, j++)
            DrawColumnLabel(i, labels[j]);
        
        if (state.Selecting)
            DrawSelectionBox(time, state.SelectionStartColumn - state.FirstColumnIndex, state.SelectionEndColumn - state.FirstColumnIndex, state.SelectionStartTime, state.SelectionEndTime);

        var selectedIndicesPerColumn = state.SelectedIndicesPerColumn;

        for (int i = 0, j = state.FirstColumnIndex; i < columnCount; i++, j++)
            DrawOnOffEventsInColumn(onOffEvents.GetElementsInColumn(j), selectedIndicesPerColumn[j], time, i, state.ShowValues);
        
        DrawModeLabel($"Mode: Events    Index: {state.Column:X2}");
    }
    ...
    private void DrawControlCurves(long time, float timeAsFloat, float[] beatArray, IReadOnlySequence<ControlKeyframe> controlCurves,
        ...
        for (int i = 0, j = state.FirstColumnIndex; i < columnCount; i++, j++)
            DrawControlCurve(controlCurves.GetElementsInColumn(j), selectedIndicesPerColumn[j], time, i, state.ShowValues);

        DrawModeLabel($"Mode: Curves    Index: {state.Column:X2}");
    }

    private void DrawModeLabel(string text) => GUI.Label(new Rect(SIDE_PADDING + FIELD_PADDING, TOP_PADDING, canvasRect.width, INFO_LABEL_HEIGHT), text);
    ...
    private void DrawSimpleField(TextFieldState field, string name, string label, int row) {
        ...
        DrawField(field, name, x + FIELD_LABEL_WIDTH, y, canvasRect.width - FIELD_LABEL_WIDTH - 2 * FIELD_PADDING, FIELD_HEIGHT);
    }
    ...
    private void DrawPaletteField(TextFieldState field, string name, string label, int row, Color color) {
        ...
        GUI.Label(new Rect(x, y, FIELD_LABEL_WIDTH, FIELD_HEIGHT), label);
        DrawField(field, name, x + FIELD_LABEL_WIDTH, y, canvasRect.width - FIELD_LABEL_WIDTH - 2 * FIELD_PADDING - FIELD_HEIGHT, FIELD_HEIGHT);
        DrawRect(canvasRect.xMax - FIELD_PADDING - FIELD_HEIGHT, y, FIELD_HEIGHT, FIELD_HEIGHT, color, false);
    }
    ...
    private void DrawHorizontalLine(int y, Color color) => DrawRect(canvasRect.x, y, canvasRect.width, 1, color, false);

    private void DrawColumnBox(int column, Color color) => DrawRect(ColumnToX(column), canvasRect.y, columnWidth, canvasRect.height, color, false);
    ...
    private void DrawSelectionBox(long time, int startColumn, int endColumn, long startTime, long endTime) {
        ...
        int startX = Mathf.Clamp(ColumnToX(startColumn), canvasRect.x, canvasRect.xMax);
        int endX = Mathf.Clamp(ColumnToX(endColumn + 1), canvasRect.x, canvasRect.xMax);
        int startY = Mathf.Clamp(RelativeTimeToY(TICK_TO_TIME * (startTime - time)) - 1, canvasRect.y, canvasRect.yMax);
        int endY = Mathf.Clamp(RelativeTimeToY(TICK_TO_TIME * (endTime - time)), canvasRect.y, canvasRect.yMax);

        DrawRect(startX, startY, endX - startX, endY - startY, SELECTION_BOX_COLOR, true);
    }
    ...
    private void DrawSustainLine(float relativeStartTime, float relativeEndTime, int column) {
        int startY = Mathf.Clamp(RelativeTimeToY(relativeStartTime) - ON_EVENT_HEIGHT, canvasRect.y, canvasRect.yMax);
        int endY = Mathf.Clamp(RelativeTimeToY(relativeEndTime), canvasRect.y, canvasRect.yMax);
    ...
    private void DrawOnOffEventsInColumn(IReadOnlyList<OnOffEvent> onOffEvents, List<int> selectedIndices, long time, int column, bool showValues) {
        ...
        for (int i = 0; i < onOffEvents.Count; i++) {
            ...
            if (relativeTime >= 0f && relativeTime <= TIMESPAN)
                DrawOnOffEvent(relativeTime, onOffEvent.Type, onOffEvent.Value, column, selectedIndices.Contains(i), showValues);
        }
    ...
    private void DrawControlCurve(IReadOnlyList<ControlKeyframe> keyframes, List<int> selectedIndices, long time, int column, bool showValues) {
        ...
        controlCurveTexture.Apply();
        GUI.DrawTexture(new Rect(ColumnToX(column) + CONTROL_CURVE_PADDING, canvasRect.y, controlCurveTexture.width, controlCurveTexture.height), controlCurveTexture, ScaleMode.StretchToFill, true);
    ...
    private int ColumnToX(int column) => columnWidth * column + canvasRect.x;

    private int RelativeTimeToY(float relativeTime) => canvasRect.yMax - Mathf.RoundToInt(pixelsPerSecond * relativeTime);
